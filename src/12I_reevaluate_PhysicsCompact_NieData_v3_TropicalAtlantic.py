# -*- coding: utf-8 -*-
r"""
12I_reevaluate_PhysicsCompact_NieData_v3_TropicalAtlantic.py

Stage 12I
=========
Fit Physics-Compact-NieData v3 on all three development missions using the
fixed epoch schedule derived from Stage 12H, then evaluate it on Tropical
Atlantic using the supplied dataset protocol.
...
Equations in standardized target space:
    W_hat_z = W_ridge_z
    V_hat_z = V_ridge_z + sigmoid(gV) * DeltaV_z

Apparent wind in physical units:
    A_hat = W_hat - V_hat

The wind branch is frozen to Ridge and has no trainable residual head. Only the
vessel branch is corrected by the compact GRU residual network.
...
Training settings:
    ...
    Adam, learning rate 1e-3, batch 256

The number of final all-development training epochs is derived only from the
completed Stage-12H LOMO runs. Tropical Atlantic is not used for gradient
training, scaler fitting, Ridge fitting or epoch selection in this script.

The script reports Persistence, Ridge and PhysicsCompactNieDataV3 under the same
supplied Tropical Atlantic samples, together with the published BP-STGNN wind
metrics as a literature reference.
"""
...
SCRIPT_VERSION = "0.3.0-PhysicsCompact-NieData-v3-RidgeWind-VesselResidual-Tropical-REEVAL"
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "data"
    / "forecasting"
    / "12I_PhysicsCompact_NieData_v3_TropicalAtlantic_REEVAL_v0_1"
)
...
def validate_12c2_freeze(
    ...
    if "v3.0" not in model_name.lower():
        raise RuntimeError(
            "Stage-12H freeze file does not identify Physics-Compact-NieData v3.0."
        )

    variants = obj.get("variants", {})
    proposed = variants.get("PhysicsCompactNieDataV3")
    if proposed is None:
        raise RuntimeError(
            "Stage-12H freeze file does not contain PhysicsCompactNieDataV3."
        )

    lambda_aw = float(proposed.get("lambda_AW", np.nan))
    if not np.isfinite(lambda_aw) or abs(lambda_aw - FREEZE.lambda_aw) > 1e-12:
        raise RuntimeError(
            f"Stage-12H lambda_AW mismatch: {lambda_aw} vs {FREEZE.lambda_aw}"
        )

    equations = " ".join(
        str(x) for x in obj.get("architecture", {}).get("equations", [])
    ).lower()
    if "w_hat_z = w_ridge_z" not in equations:
        raise RuntimeError("Stage-12H freeze does not confirm frozen Ridge wind.")
    if "v_hat_z = v_ridge_z" not in equations:
        raise RuntimeError("Stage-12H freeze does not confirm Ridge vessel anchor.")
...
def train_final_seed(
    *,
    ...
    checkpoint_path = (
        checkpoint_dir
        / f"PhysicsCompactNieDataV3_seed_{seed}.pt"
    )

    history_path = (
        history_dir
        / f"PhysicsCompactNieDataV3_seed_{seed}.csv"
    )
    ...
    torch.save({
        ...
        "anchor": "Frozen Ridge wind + Ridge vessel anchor; vessel-only residual",
        "seed": int(seed),
...
def final_checkpoint_valid(
    ...
    checks = [
        ...
        == "Frozen Ridge wind + Ridge vessel anchor; vessel-only residual",
        int(
            ckpt.get(
                "seed",
                -1,
...
def summarize_seed_results(
    ...
    out = {
        "method": "PhysicsCompactNieDataV3",
        "seed_count": int(
            len(
                seed_df
            )
...
def main():
    ...
    log(
        "model                 : Physics-Compact-NieData v3.0"
    )
    log(
        "anchor                : frozen Ridge wind + Ridge vessel anchor"
    )
    log(
    ...
    preflight_manifest = {
        ...
        "Stage12H_freeze_path": str(
            c2_freeze_path
        ),
        "Stage12H_results_path": str(
            c2_results_path
        ),
        "comparison_scope": (
    ...
    if args.preflight_only:
        log("")
        log("=" * 126)
        log("12I PREFLIGHT PASSED")
        log("=" * 126)
        log(
            "[PASS] Stage-12H v3 freeze validated."
        )
        log(
    ...
    access_record = {
        "stage": "12I",
        "event": "Tropical_Atlantic_V3_REEVALUATION_OPEN",
    ...
    log("")
    log("=" * 126)
    log("[PHASE C] OPENING TROPICAL ATLANTIC V3 RE-EVALUATION")
    log("=" * 126)
    log(
    ...
    # v3 neural model uses ridge_test_z as its frozen anchor.
    ...
    for plan_row in epoch_plan.itertuples():
        ...
        np.savez_compressed(
            prediction_dir
            / f"PhysicsCompactNieDataV3_seed_{seed}_TropicalAtlantic.npz",
            y_true=test["y"],
    ...
    with (
        ...
        f.write(
            "12D FINAL Physics-Compact-NieData v3.0 "
    ...
    log("")
    log(
        "PhysicsCompactNieDataV3 skill vs Persistence: "
    ...
    log(
        "PhysicsCompactNieDataV3 skill vs Ridge      : "
