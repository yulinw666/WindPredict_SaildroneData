# -*- coding: utf-8 -*-
r"""
12C_train_PhysicsCompact_NieData_LOMO.py
...
MODEL FREEZE -- Physics-Compact-NieData v1.0
--------------------------------------------
This is a data-compatible variant declared BEFORE held-out Tropical Atlantic
evaluation. It keeps the compact residual philosophy but adds a wind residual
branch so the wind predictor itself can be evaluated against published
BP-STGNN wind metrics.
...
the fold-specific Ridge prediction.
...
The proposed Stage-12D model is PhysicsCompactNieData. CompactNoPhysics is an
...
STAGE-12D POLICY
----------------
Stage 12C does NOT fit the final all-development model and does NOT touch the
held-out Tropical Atlantic mission.

If Stage 12C completes cleanly, Stage 12D will:
...
python "D:\project\WindPredict_SaildroneData\src\12C_train_PhysicsCompact_NieData_LOMO.py"

Optional NON-SCIENTIFIC smoke:
python "...12C_train_PhysicsCompact_NieData_LOMO.py" --debug-fast
"""
...
SCRIPT_VERSION = "0.1.0-PhysicsCompact-NieData-LOMO"
...
NEURAL_VARIANTS = {
    "CompactNoPhysics": 0.0,
    "PhysicsCompactNieData": FREEZE.lambda_aw_physics,
}
...
def build_model_class(torch, nn, F):
    class PhysicsCompactNieData(nn.Module):
        def __init__(self):
            super().__init__()
...
def main():
    ...
    output_dir = (
        ...
        / "forecasting"
        / (
            "12C_PhysicsCompact_NieData_LOMO_v0_1_DEBUG_FAST"
            if args.debug_fast
            else "12C_PhysicsCompact_NieData_LOMO_v0_1"
        )
    )
    ...
    log("=" * 124)
    log("12C - PHYSICS-COMPACT-NIEDATA DEVELOPMENT-ONLY LOMO")
    log("=" * 124)
    ...
    freeze_payload = {
        "stage": "12C",
        "model_name": "Physics-Compact-NieData v1.0",
        "script_version": SCRIPT_VERSION,
        ...
        "architecture": {
            ...
            "model_freeze": asdict(FREEZE),
            "ridge_alpha": RIDGE_ALPHA,
            "delta_head_initialization": "all zeros; neural model starts at Ridge",
        },
        "variants": {
            "CompactNoPhysics": {
                "lambda_AW": 0.0,
    ...
    if full_complete and not method_summary.empty:
        ...
        proposed = method_summary.loc[
            method_summary["method"] == "PhysicsCompactNieData"
        ]
        ...
        if not proposed.empty and not ridge_summary.empty:
            ...
            log("")
            log(
                f"PhysicsCompactNieData skill vs Ridge: "
    ...
    log(
        "[POLICY] PhysicsCompactNieData is the proposed Stage-12D model; "
        "CompactNoPhysics is ablation only."
    )
