# -*- coding: utf-8 -*-
r"""
11B_plot_Fig08_external_branch_decomposition_singlepanel_v3.py

Updated according to the latest request:
- keep only one panel: apparent-wind vector RMSE
- four dataset groups:
...
- four bars per group:
    ...
    Motion enhanced
    Physics-Compact
- use the color family of the user's reference bar chart
- add numeric labels above each bar
- first x-axis label becomes:
    SD1090 2024
    internal evaluation
"""
...
# Palette adapted from the reference chart:
# blue / orange / green as the dominant family, plus a harmonized red.
COLORS = {
    "Dual Ridge": "#4C78A8",        # blue
    "Wind enhanced": "#F58518",     # orange
    "Motion enhanced": "#54A24B",   # green
    "Physics-Compact": "#E45756",   # muted red
}
    ...
    if style_path.exists():
        try:
            spec = importlib.util.spec_from_file_location(
                "sci_plot_style_fig08_singlepanel_v3",
                str(style_path),
            )
...
def add_value_labels(ax, bars, values, dy=0.018, fontsize=7.2):
...
def plot_figure(table: pd.DataFrame, output_dir: Path):
    ...
    for bars, vals in zip(all_bars, all_vals):
        add_value_labels(ax, bars, vals, dy=0.018, fontsize=7.0)
    ...
    base = output_dir / "Fig08_branch_transfer_AW_RMSE_singlepanel"
...
def main():
    ...
    internal_csv = discover_internal_csv(dataset_dir, args.internal_csv)
    external_csv = discover_external_csv(external_dir, args.external_csv)

    log("=" * 100)
    log("FIG. 8 — INTERNAL + CROSS-VOYAGE BRANCH COMPARISON (SINGLE PANEL)")
    log("=" * 100)
    log(f"root         : {root}")
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    table.to_csv(
        output_dir / "Fig08_branch_transfer_AW_RMSE_singlepanel_data.csv",
        index=False,
        encoding="utf-8-sig",
    )

    plot_figure(table, output_dir)
