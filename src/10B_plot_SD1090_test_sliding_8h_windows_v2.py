# -*- coding: utf-8 -*-
r"""
10B_plot_SD1090_test_sliding_8h_windows_v2.py
...
5) produces one SCI-style 6x1 figure per window:
       (a) U_app at 1 min
       (b) V_app at 1 min
       (c) U_app at 5 min
       (d) V_app at 5 min
   ...
   Every subplot uses the SAME horizontal width as the original 8-h
   2x1 figure, so the time-series details remain readable.
   with:
       Observed
       Physics-Compact
       DLinear
...
python "D:\project\WindPredict_SaildroneData\src\10B_plot_SD1090_test_sliding_8h_windows_v2.py" `
...
def plot_window_6x1(
    ...
    """
    One full-width six-panel figure per 8-h window.
    ...
    The horizontal width is kept at 7.15 in, exactly matching the previous
    8-h 2x1 manuscript figure.  Only the vertical size is increased.
    """
    start_ns = int(window["start_time_ns"])
    end_ns = int(window["end_time_ns"])

    # Keep the same horizontal width as the original 8-h figure.
    # Increase only height to accommodate six readable panels.
    ...
    # ------------------------------------------------------------------
    # Build all panel data first.
    # This also lets us use common U_app and V_app y-limits across horizons.
    # ------------------------------------------------------------------
    panel_data = {}
    all_u = []
    all_v = []
    ...
    for h in PLOT_HORIZONS:
        ...
        pred_app = {
            model: apparent_from_joint(
                pred_mean[model][a:b, j, :]
            )
            for model in MODEL_ORDER
        }
        ...
        for model in MODEL_ORDER:
            all_u.append(pred_app[model][:, 0])
            all_v.append(pred_app[model][:, 1])

    def global_limits(series_list):
        lo = min(
            float(np.nanmin(x))
    ...
    u_ylim = global_limits(all_u)
    v_ylim = global_limits(all_v)
    ...
    for h in PLOT_HORIZONS:
        obs = panel_data[h]["obs"]
        pred_app = panel_data[h]["pred"]
        ...
        # Put the forecast horizon inside the panel rather than above it,
        # preserving vertical space and matching the compact SCI style.
        ax_u.text(
            0.985,
            0.965,
            f"{h}-min horizon",
    ...
    axes[-1].set_xlabel(
        "Time within 8-h window (h)"
    )

    # Fixed 0–8 h ticks make all windows visually consistent.
    axes[-1].set_xlim(
        0.0,
        (len(panel_data[10]["obs"]) - 1) / 60.0,
    )

    # One shared legend at the top, same order/colors as the previous figure.
    ...
    fig.suptitle(
        f"{selected_prefix}SD1090 internal evaluation: "
