# -*- coding: utf-8 -*-
r"""
10C_SD1090_forecast_lag_diagnostic_v2.py
...
----------------------
python "D:\project\WindPredict_SaildroneData\src\10C_SD1090_forecast_lag_diagnostic_v2.py" `
...
def build_prediction_sets(
    ...
    Persistence / Dual Ridge.

    IMPORTANT
    ---------
    10A-v4's formal Dual Ridge is NOT the sklearn-Ridge reconstruction used
    by the old helper ``fit_final_ridges``.  The final Physics-Compact
    pipeline uses the deterministic centered/standardized Ridge formulation
    from 15E/15F:

        TrueWind Ridge: alpha_W = 0
        Motion/HDG joint Ridge: alpha_M = 1500

    The two implementations are mathematically nearly equivalent but can
    differ by ~1e-4 to 1e-3 m/s because of numerical formulation and target
    standardization.  The lag diagnostic must reproduce 10A at lag=0
    exactly, so v2 rebuilds the SAME anchors as 10A-v4 / 15F.
    """
    ...
    # -----------------------------------------------------------------
    # Persistence: use the exact 10A implementation.
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # Dual Ridge: rebuild with EXACT final 15E/15F formulation.
    # -----------------------------------------------------------------
    src_dir = root / "src"
    ...
    emod = load_module(
        ...
        "lag_v2_stage15e_helper",
    )

    fmod = load_module(
        src_dir
        / "15F_frozen_15EVH_alpha1500_external_generalization.py",
        "lag_v2_stage15f",
    )

    # Synchronize the helper with the FINAL selected alpha_M.
    final_motion_alpha = float(
        getattr(
            ten_a,
            "FINAL_MOTION_ALPHA",
            1500.0,
    ...
    if not np.isclose(
        ...
        log(
            f"[WARNING] 10A FINAL_MOTION_ALPHA={final_motion_alpha:g}; "
            "expected 1500 from the frozen paper model."
        )
    ...
    if not np.isclose(
        ...
        log(
            f"[WARNING] 10A FINAL_WIND_ALPHA={final_wind_alpha:g}; "
            "expected 0 from the frozen paper model."
        )

    emod.OLD_RIDGE_ALPHA = (
        final_motion_alpha
    )
    ...
    test_truth_exact = np.asarray(
        test["y"],
        dtype=np.float32,
    )

    if (
        test_truth_exact.shape
        != truth.shape
        or not np.allclose(
            test_truth_exact,
            truth,
        ...
        raise RuntimeError(
            "Final 15E/15F TEST truth does not align with 10A truth."
        )

    # Final joint motion/heading Ridge, alpha_M=1500.
    old_joint_ridge = (
        emod.fit_old_joint_ridge(
            train[
                "X"
            ],
            train[
    ...
    # Same heading-pair normalization as 10A-v4 / 15F.
    ridge_motion[
        :,
        :,
        2:
        4
    ] = fmod.normalize_heading_pair(
        ridge_motion[
            :,
            :,
            2:
    ...
    # Final dedicated TrueWind Ridge, alpha_W=0.
    truewind_ridge = (
        fmod.fit_truewind_ridge(
            emod,
            train[
                "X"
            ],
    ...
    log("")
    log(
        "[DUAL RIDGE RECONSTRUCTION]"
    )
    log(
        f"  formulation = exact final 15E/15F deterministic anchors"
    )
    log(
        f"  alpha_W     = {final_wind_alpha:g}"
    )
    log(
        f"  alpha_M     = {final_motion_alpha:g}"
    )

    # -----------------------------------------------------------------
    # Frozen modern baselines.
    # -----------------------------------------------------------------
    for model in [
        "DLinear",
        "TimeMixer",
    ...
    # -----------------------------------------------------------------
    # Frozen Physics-Compact predictions.
    # -----------------------------------------------------------------
    result[
        "Physics-Compact"
    ] = load_pc_predictions(
...
def write_report(
    ...
    lines.append(
        "10C-v2 — SD1090 FORECAST-LAG DIAGNOSTIC"
    )
    ...
    lines.append(
        "  Dual Ridge is reconstructed with the exact final 15E/15F deterministic "
        "centered/standardized formulation (alpha_W=0, alpha_M=1500)."
    )
