# -*- coding: utf-8 -*-
"""
15C_S_train_1min_VesselHDG_Residual_ApparentWind_SingleGRU.py
...
Ablation purpose
----------------
This is the SINGLE-GRU version of Stage15C.

For a fair comparison with the 2-layer Stage15C, the following are unchanged:
    - dataset and train/validation/test split
    - Stage15B-F true-wind upstream predictions
    - Stage15A-F Vessel+HDG Ridge anchor (alpha=2250)
    - 60 x 11 history input
    - future upstream context
    - residual targets
    - shared MLP size
    ...
    - early stopping rule
    - validation score
    - horizon-specific alpha_E / alpha_N / alpha_HDG
    - random seed
    - test firewall

ONLY the GRU depth changes:
    2 layers -> 1 layer

PyTorch GRU internal dropout is automatically disabled for a single layer,
while the shared MLP dropout remains 0.10, exactly as in the 2-layer script.

Recommended PowerShell
----------------------
python "D:\\project\\WindPredict_SaildroneData\\src\\15C_S_train_1min_VesselHDG_Residual_ApparentWind_SingleGRU.py" --dataset-dir "D:\\project\\WindPredict_SaildroneData\\data\\forecasting\\SD1090_TPOS2024_JointForecasting_v0_1" --stage15b-dir "D:\\project\\WindPredict_SaildroneData\\data\\forecasting\\15B_F_1min_TrueWind_Residual_v0_3" --stage15a-vessel-dir "D:\\project\\WindPredict_SaildroneData\\data\\forecasting\\15A_F_VesselHDG_alpha2250_v0_3" --output-dir "D:\\project\\WindPredict_SaildroneData\\data\\forecasting\\15C_S_1min_VesselHDG_ApparentResidual_SingleGRU_v0_1"
"""
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "data"
    / "forecasting"
    / "15C_S_1min_VesselHDG_ApparentResidual_SingleGRU_v0_1"
)
...
def main():
    ...
    log("=" * 150)
    log(
        "15C-S — STEP 3: VESSEL/HDG RESIDUAL + APPARENT-WIND PHYSICAL COUPLING"
    )
    log("=" * 150)
    ...
    log(
        f"Residual network: single-layer GRU, hidden={GRU_HIDDEN}"
    )
    ...
    log("")
    log(
        "[STAGE 4/8] Training single-layer Vessel/HDG residual GRU."
    )
    ...
    np.savez_compressed(
        out
        / "15C_S_validation_predictions.npz",
    ...
    checkpoint_path = (
        out
        / "15C_S_vessel_hdg_residual_single_gru.pt"
    )
    ...
    with (
        ...
        f.write(
            "15C-S — SINGLE-GRU VESSEL/HDG RESIDUAL + APPARENT-WIND COUPLING\n"
        )
    ...
    log("=" * 150)
    log(
        "15C-S SINGLE-GRU FINAL TEST — VESSEL/HDG + APPARENT WIND"
    )
    log("=" * 150)
