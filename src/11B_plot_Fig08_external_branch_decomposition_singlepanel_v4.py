# -*- coding: utf-8 -*-
r"""
11B_plot_Fig08_external_branch_decomposition_singlepanel_v4.py

Updates from v3:
1) Restore the original Fig. 8 color family:
   - Dual Ridge      : orange
   - Wind enhanced   : green
   - Motion enhanced : purple
   - Physics-Compact : red
2) Restore SD1033 in the x-axis labels.
3) Tighten the y-axis range to match the original single-panel style
   (default: 0 to 1.16 instead of auto-expanding too high).
4) Keep numeric labels above bars.
"""
...
# Restored original-like Fig. 8 palette
COLORS = {
    ...
    if style_path.exists():
        try:
            spec = importlib.util.spec_from_file_location(
                "sci_plot_style_fig08_singlepanel_v4",
                str(style_path),
            )
...
def discover_internal_csv(root: Path, dataset_dir: Path, explicit: Path | None) -> Path:
    ...
    candidates = [
        root / "Table_internal_branch_ablation.csv",
        dataset_dir / "Table_internal_branch_ablation.csv",
        root / "data" / "forecasting" / "10A_SD1090_internal_evaluation_v0_4" / "tables" / "Table_internal_branch_ablation.csv",
        root / "data" / "forecasting" / "10A_SD1090_internal_evaluation_v0_3" / "tables" / "Table_internal_branch_ablation.csv",
    ]
    ...
    found = list(root.rglob("Table_internal_branch_ablation.csv"))
    if found:
        return sorted(found, key=lambda p: len(str(p)))[0]

    raise FileNotFoundError(
        "Could not find Table_internal_branch_ablation.csv.\n"
        f"Search root: {root}"
    )


def discover_external_csv(root: Path, external_dir: Path, explicit: Path | None) -> Path:
    if explicit is not None:
        p = explicit.resolve()
        if not p.exists():
            raise FileNotFoundError(p)
    ...
    candidates = [
        external_dir / "15F_external_branch_decomposition.csv",
        root / "15F_external_branch_decomposition.csv",
    ]
    ...
    found = list(root.rglob("15F_external_branch_decomposition.csv"))
    if found:
        return sorted(found, key=lambda p: len(str(p)))[0]

    raise FileNotFoundError(
        "Could not find 15F_external_branch_decomposition.csv.\n"
        f"Search root: {root}"
    )
...
def build_table(internal_df: pd.DataFrame, external_df: pd.DataFrame) -> pd.DataFrame:
    table = pd.concat([internal_df, external_df], ignore_index=True)
...
def add_value_labels(ax, bars, values, dy=0.012, fontsize=7.0):
...
def plot_figure(table: pd.DataFrame, output_dir: Path, y_min: float, y_max: float):
    fig, ax = plt.subplots(
        figsize=(7.45, 3.45),
    ...
    width = 0.18
    offsets = np.array([-1.5, -0.5, 0.5, 1.5], dtype=float) * width
    ...
    for i, model in enumerate(MODEL_ORDER):
        ...
        for ds in DATASET_ORDER:
            q = table[(table["dataset"] == ds) & (table["model"] == model)]
            if len(q) != 1:
                raise RuntimeError(
    ...
    for bars, vals in zip(all_bars, all_vals):
        add_value_labels(ax, bars, vals, dy=0.012, fontsize=7.0)
    ...
    base = output_dir / "Fig08_branch_transfer_AW_RMSE_singlepanel_v4"
...
def main():
    ...
    ap.add_argument("--internal-csv", type=Path, default=None)
    ap.add_argument("--external-dir", type=Path, default=None)
    ap.add_argument("--external-csv", type=Path, default=None)
    ap.add_argument("--output-dir", type=Path, default=None)

    # fixed y-axis range, closer to the original panel style
    ap.add_argument("--y-min", type=float, default=0.0)
    ap.add_argument("--y-max", type=float, default=1.16)
    ...
    internal_csv = discover_internal_csv(root, dataset_dir, args.internal_csv)
    external_csv = discover_external_csv(root, external_dir, args.external_csv)

    log("=" * 100)
    log("FIG. 8 — INTERNAL + CROSS-VOYAGE BRANCH COMPARISON (SINGLE PANEL, V4)")
    log("=" * 100)
    log(f"root         : {root}")
    ...
    log(f"y_range      : [{args.y_min}, {args.y_max}]")

    internal_df = load_internal(internal_csv)
    external_df = load_external(external_csv)
    table = build_table(internal_df, external_df)
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    table.to_csv(
        output_dir / "Fig08_branch_transfer_AW_RMSE_singlepanel_v4_data.csv",
        index=False,
        encoding="utf-8-sig",
    )

    plot_figure(table, output_dir, y_min=args.y_min, y_max=args.y_max)
