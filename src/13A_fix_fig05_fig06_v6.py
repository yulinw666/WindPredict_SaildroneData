# -*- coding: utf-8 -*-
"""
13A_fix_fig05_fig06_v6.py

Updates requested by the user:
1) Fig05: move "Ridge = 1.1252" to a non-overlapping position.
2) Fig06: remove the right-side external panel and place the legend/data
   inside the empty lower-left region of the plot.

This script only redraws figures; it does not retrain any model.
"""
...
def extract_fig05(root: Path):
    ...
    try:
        ...
        print(f"[WARN] Fig05 parse failed: {exc}; using fallback values.")
        return fallback_fig05()
...
def make_fig05(root: Path, outdir: Path):
    ...
    gs = GridSpec(1, 2, width_ratios=[5.0, 1.15], wspace=0.06, figure=fig)
    ...
    # fig.supxlabel(r"Validation apparent-wind vector RMSE (m s$^{-1}$)", y=0.02)
    ...
    # Put Ridge label near the top but offset from the dashed line.
    ax2.text(
        0.10, 0.97,
        f"Ridge = {ridge:.4f}",
    ...
    # 让 x 轴标题相对于“主绘图区（两个子图整体）”居中，而不是相对于整张 figure 居中
    x_center_main = (0.31 + 0.98) / 2.0
    fig.text(
        x_center_main,
        0.06,
    ...
    save_figure(fig, outdir / "Fig05_ablation_validation_AW_RMSE_v6")

    out = df.copy()
    out["ridge_reference"] = ridge
    out.to_csv(
        outdir / "Fig05_ablation_validation_AW_RMSE_v6_data.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def fallback_fig06():
    ...
    return pd.DataFrame(
        rows,
        columns=["label", "ridge_skill_pct", "sse_reduction_pct", "gamma", "corr_mag"],
    )
...
def extract_fig06(root: Path):
    ...
    try:
        vessel = ridge[ridge["branch"].astype(str).str.lower().eq("vessel")].copy()
        vessel = (
            vessel.groupby("dataset_id", as_index=False)["ridge_skill_vs_persistence"]
            .mean()
        )
        ...
        k = key[cols].copy()
        k["sse_reduction_pct"] = 100.0 * k["mean_vessel_SSE_reduction_fraction_vs_Ridge"]
        ...
        return df[["label", "ridge_skill_pct", "sse_reduction_pct", "gamma", "corr_mag"]].reset_index(drop=True)
    except Exception as exc:
        print(f"[WARN] Fig06 parse failed: {exc}; using fallback values.")
        return fallback_fig06()
...
def make_fig06(root: Path, outdir: Path):
    ...
    # Marker area proportional to |c|
    cmin = float(df["corr_mag"].min())
    cmax = float(df["corr_mag"].max())
    if cmax > cmin:
        df["marker_area"] = 110.0 + 320.0 * (df["corr_mag"] - cmin) / (cmax - cmin)
    else:
        df["marker_area"] = 180.0
    ...
    ax.set_xlim(
        ...
        max(8.0, float(df["ridge_skill_pct"].max()) + 3.5),
    )
    ax.set_ylim(
    ...
    # Compact legend/data box INSIDE lower-left blank area.
    box = ax.inset_axes([0.03, 0.045, 0.60, 0.37])
    ...
    box.text(0.05, 0.89, "Dataset", ha="left", va="center", fontweight="bold", fontsize=9.0)
    box.text(0.77, 0.89, r"$\gamma$", ha="center", va="center", fontweight="bold", fontsize=9.0)
    box.text(0.93, 0.89, r"$|c|$", ha="center", va="center", fontweight="bold", fontsize=9.0)
    box.plot([0.04, 0.96], [0.80, 0.80], color="0.2", linewidth=0.6)

    yvals = [0.72, 0.54, 0.36, 0.18]
    for yy, row in zip(yvals, df.itertuples(index=False)):
        box.scatter(
            0.09, yy, s=80,
            color=color_map[row.label],
            edgecolor="black",
        ...
        box.text(0.17, yy, row.label, ha="left", va="center", fontsize=8.6)
        box.text(0.77, yy, f"{row.gamma:.2f}", ha="center", va="center", fontsize=8.6)
        box.text(0.93, yy, f"{row.corr_mag:.2f}", ha="center", va="center", fontsize=8.6)

    box.text(0.93, 0.07, r"m s$^{-1}$", ha="center", va="center", fontsize=8.0)
    box.text(0.05, 0.07, r"Marker area $\propto |c|$", ha="left", va="center", fontsize=8.0)

    fig.subplots_adjust(left=0.11, right=0.98, bottom=0.17, top=0.98)
    save_figure(fig, outdir / "Fig06_mechanism_vessel_shift_correction_v6")

    df.to_csv(
        outdir / "Fig06_mechanism_vessel_shift_correction_v6_data.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def main():
    ...
    print("=" * 100)
    print("REDRAW FIG05 + FIG06 — v6")
    print("=" * 100)

    make_fig05(root, outdir)
    make_fig06(root, outdir)

    print("\n[DONE] Fig05 v6 and Fig06 v6 generated.")
