...
MODEL_LABELS = {
    "15E-VH": "Physics-Compact",
    "physiccompact": "Physics-Compact",
    "physicscompact": "Physics-Compact",
    "physics-compact": "Physics-Compact",
    "dualridge": "Dual Ridge",
    "ridge": "Dual Ridge",
    "windenhanced+ridgemotion": "Wind enhanced",
    "ridgewind+motionenhanced": "Motion enhanced",
    "dlinear": "DLinear",
    "timemixer": "TimeMixer",
...
COLORS = {
    "Observed": "#000000",
    "Dual Ridge": "#0072B2",
    "Physics-Compact": "#D55E00",
    "Wind enhanced": "#009E73",
    "Motion enhanced": "#CC79A7",
    "DLinear": "#009E73",
    "TimeMixer": "#CC79A7",
    "iTransformer": "#56B4E9",
    "PatchTST": "#E69F00",
...
def canonical_model(x) -> str:
    """
    Canonicalize model names after removing spaces / '_' / '-' / '+' etc.

    Important:
    MODEL_LABELS contains several human-readable keys such as
    "15E-VH", "windenhanced+ridgemotion", and
    "ridgewind+motionenhanced".  The previous implementation normalized
    the input name but did NOT normalize the dictionary keys, so these
    three final branch variants were silently missed and Fig. 6 plotted
    only Dual Ridge.
    """
    n = norm_text(x)

    # Normalize BOTH the input and every mapping key.
    normalized_labels = {
        norm_text(key): value
    ...
    if n in normalized_labels:
        return normalized_labels[n]

    # Tolerant substring matching on normalized strings.
...
def plot_fig2(
    ...
    for d in seed_data:
        ...
        app_wind_only.append(
            vector_rmse_per_horizon(true_app, app_w)
        )
        app_motion_only.append(
            vector_rmse_per_horizon(true_app, app_m)
        )
        app_full.append(
            vector_rmse_per_horizon(true_app, app_f)
        )
    ...
    ax.errorbar(
        ...
        linestyle=LINESTYLES["Physics-Compact"],
    ...
    # (c) Apparent wind branch decomposition
    ax = axes[2]
...
def plot_fig4(
    ...
    # Final proposed validation horizon table.
    vh_csv = final_vh_dir / "15E_VH_per_horizon_all_seeds.csv"
    if not vh_csv.exists():
        raise FileNotFoundError(vh_csv)

    vh = pd.read_csv(vh_csv)
    proposed = model_horizon_table(
        vh,
        [
            "AppVector_RMSE_mps",
            "apparent_vector_RMSE_mps",
    ...
    # Some files call the model "15E-VH"; canonical_model maps it.
    if proposed.empty:
        model_col = find_col(vh, ["model", "method"])
        vh["_canon"] = vh[model_col].map(canonical_model)
        proposed = model_horizon_table(
            vh.loc[vh["_canon"] == "Physics-Compact"],
            [
                "AppVector_RMSE_mps",
                "apparent_vector_RMSE_mps",
                "AW_RMSE_mps",
        ...
        proposed["model"] = "Physics-Compact"
    ...
    combined = pd.concat([proposed, tab], ignore_index=True)

    for name in order:
        d = combined[combined["model"] == name].sort_values("horizon_min")
        if d.empty:
            continue
...
def plot_fig6(output_dir: Path, final_ext_dir: Path):
    ...
    # Strict audit: every external dataset must contain all four branch
    # variants after canonicalization.  Stop immediately instead of
    # silently drawing NaN / missing bars.
    print("\n" + "=" * 100)
    print("FIG. 6 CANONICALIZED SOURCE CHECK")
    print("=" * 100)
    print("Raw model names:")
    print(df[model_col].drop_duplicates().to_string(index=False))
    print("\nCanonical model names:")
    print(df["_model"].drop_duplicates().to_string(index=False))

    missing_pairs = []
    for ds in dataset_order:
        wanted = norm_text(ds)
        ds_mask = df["_dataset"].map(norm_text).map(
            lambda s: wanted in s or s in wanted
        )
        for model in models:
            if not np.any(ds_mask & (df["_model"] == model)):
                missing_pairs.append((ds, model))

    if missing_pairs:
        msg = "\n".join(
            f"  dataset={ds}, model={model}"
            for ds, model in missing_pairs
        )
        raise RuntimeError(
            "Fig. 6 is missing required dataset/model combinations after "
            "canonicalization:\n" + msg
        )

    print("\nAll 3 datasets x 4 branch variants are present.")
    print("=" * 100)
