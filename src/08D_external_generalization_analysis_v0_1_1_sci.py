...
Plot style
----------
STRICTLY uses sci_plot_style.py from the same src directory as the single
source of truth for all publication figure formatting.

The script:
    - requires sci_plot_style.py to exist;
    - explicitly calls apply_sci_style(base_font_size=8.5);
    - does NOT override the style-file rcParams afterwards;
    - uses style.clean_axis()/finish_axes();
    - uses style.save_figure() for 600 dpi PNG + vector PDF;
    - uses only colors defined by style.PALETTE;
    - uses compact journal-sized figures (3.6-in single column or
      7.2-in double column where needed);
    - avoids large in-figure titles.

If sci_plot_style.py cannot be loaded/applied, plotting stops rather than
silently falling back to inconsistent formatting.
"""
...
SCRIPT_VERSION = "0.1.1-strict-sci-style"
...
def load_style():
    """
    Strictly load and APPLY sci_plot_style.py.

    The user's shared sci_plot_style.py is the single source of truth for:
        font selection,
        font sizes,
        tick directions,
        line widths,
        ...
        600 dpi PNG export,
        vector PDF export,
        and the project color palette.

    No plotting fallback is permitted here because 08D produces
    ...
    if not style_path.exists():
        raise FileNotFoundError(
            ...
            f"{style_path}\n"
            "Place sci_plot_style.py in the same src directory as 08D."
        )
    ...
    log(
        f"[SCI STYLE] Loaded and APPLIED: {style_path}"
    )

    if selected_font is not None:
        log(
            f"[SCI STYLE] selected_font={selected_font}"
        )
    else:
        log(
            "[SCI STYLE] "
    ...
    log(
        ...
        f"axes.labelsize={plt.rcParams.get('axes.labelsize')} | "
        f"xtick.labelsize={plt.rcParams.get('xtick.labelsize')} | "
        f"legend.fontsize={plt.rcParams.get('legend.fontsize')}"
    )
    ...
    log(
        ...
        f"savefig.bbox={plt.rcParams.get('savefig.bbox')}"
    )

    return module


def palette_value(style, candidates, default=None):
    """
    Read colors ONLY from sci_plot_style.PALETTE.

    candidates are tried in order so the function works with both
    project palette versions previously used in this work.
    """
    palette = getattr(
        style,
        "PALETTE",
    ...
    raise KeyError(
        "None of the requested palette keys exist in "
        f"sci_plot_style.PALETTE: {candidates}"
    )


def line_style_value(style, candidates, default="-"):
    """
    Read line styles from either LINE_STYLES or LINESTYLES, supporting
    both project versions of sci_plot_style.py.
    """
    table = getattr(
        style,
        "LINE_STYLES",
...
def marker_value(style, candidates, default="o"):
    table = getattr(
        style,
        "MARKERS",
        {},
...
def style_axis(
    ...
    *,
    ...
    if hasattr(
        ...
        raise AttributeError(
            "sci_plot_style.py must define clean_axis() "
            "or finish_axes()."
        )
...
def style_save(
    ...
    if not hasattr(
        ...
        raise AttributeError(
            "sci_plot_style.py must define save_figure()."
        )
    ...
    log(
        f"[SCI FIGURE] {base.name}.png / {base.name}.pdf"
    )
...
def make_figures(
    *,
    ...
    # Force a non-interactive batch backend BEFORE importing pyplot or
    # loading the style module (which itself imports pyplot).
    import matplotlib
    matplotlib.use(
        "Agg",
        force=True,
    ...
    # ------------------------------------------------------------------
    # Project palette from sci_plot_style.py only.
    # ------------------------------------------------------------------
    color_persistence = palette_value(
        style,
        [
            "background",
            "baseline",
    ...
    model_colors = {
        MODEL_PERSISTENCE: color_persistence,
        MODEL_RIDGE: color_ridge,
        MODEL_COMPACT: color_compact,
        MODEL_PHYSICS: color_physics,
    }

    model_markers = {
        MODEL_PERSISTENCE: marker_value(
            style,
            ["ohats"],
            "o",
        ),
        MODEL_RIDGE: marker_value(
            style,
            ["stage3"],
            "s",
        ),
        MODEL_COMPACT: marker_value(
            style,
            ["proposed_no_wave"],
            "D",
        ),
        MODEL_PHYSICS: marker_value(
            style,
            ["proposed"],
            "^",
        ),
    ...
    # ==================================================================
    # Fig. 1: Primary external AW RMSE
    # Double-column width because four models are compared for 3 datasets.
    # ==================================================================
    fig, ax = plt.subplots(
        figsize=(
            7.2,
    ...
    offsets = {
        MODEL_PERSISTENCE: -1.5 * width,
        MODEL_RIDGE: -0.5 * width,
        MODEL_COMPACT: 0.5 * width,
        MODEL_PHYSICS: 1.5 * width,
    }
    ...
    # ==================================================================
    # Fig. 2: Per-horizon AW skill
    # Single-column compact figure.
    # ==================================================================
    fig, ax = plt.subplots(
        figsize=(
            3.6,
    ...
    for idx, dataset_id in enumerate(
        primary_datasets
    ):
        d = (
            horizon_skill.loc[
        ...
        ax.plot(
            ...
            color=dataset_line_colors[
                idx
                % len(
                    dataset_line_colors
                )
            ],
            linestyle=dataset_line_styles[
                idx
                % len(
                    dataset_line_styles
                )
            ],
            marker=dataset_markers[
                idx
                % len(
                    dataset_markers
                )
    ...
    # ==================================================================
    # Fig. 3: AW skill heat map
    # Uses only project palette endpoints.
    # ==================================================================
    pivot = (
        horizon_skill
        .pivot(
    ...
    # ==================================================================
    # Fig. 4/5: Matched-period cross-vehicle comparison
    # ==================================================================
    matched_id = (
        "SD1033_2024_matched_SD1090"
    )
    ...
    # ==================================================================
    # Fig. 6: Distribution shift vs AW gain
    ...
    # ==================================================================
    # Fig. 7: Vessel improvement vs Ridge
    # ==================================================================
    ...
    # ==================================================================
    # Fig. 8: Physics refinement over compact
    # ==================================================================
    physics_ref = (
...
def main():
    ...
    manifest = {
        ...
        "plot_style": {
            "strict_sci_plot_style": True,
            "style_file": str(
                Path(__file__).resolve().parent
                / "sci_plot_style.py"
            ),
            "base_font_size": 8.5,
            "single_column_figsize_in": [3.6, 2.8],
            "double_column_width_in": 7.2,
            "export": "600 dpi PNG + vector PDF via sci_plot_style.save_figure()",
        },
        "software": {
            "python": sys.version,
    ...
    log(
        "[PLOT STYLE] Strict sci_plot_style.py; "
