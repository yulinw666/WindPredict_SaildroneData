...
SCRIPT_VERSION = "0.1.1-BP-STGNN-dataset-build-empty-trackP-recovery"
...
def resample_10min_complete_blocks(
    ...
    # Preserve the expected schema even when no complete 10-min blocks exist.
    # This situation is scientifically meaningful (for example, an entire
    # required BP-STGNN variable such as pressure may be unavailable) and must
    # not crash downstream auditing.
    base_columns = [
        "block_start_ns",
        "U",
        "V",
...
def subset_track_p_to_time_domain(
    ...
    Empty Track-P data are valid audit outcomes. They can occur when a paper-
    required variable is unavailable over an entire mission. In that case we
    return an empty dataframe with the expected schema rather than failing.
    """
    expected_columns = [
        "block_start_ns",
        "U",
    ...
    if (
        ...
        raise KeyError(
            "Track-P resampled dataframe is non-empty but has no "
            "'block_start_ns' column. This indicates a preprocessing bug."
        )
...
def main():
    ...
    for key, path in raw_map.items():
        ...
        if len(
            ...
            log(
                f"  [WARN] {key}: zero complete 10-min BP-STGNN blocks. "
                "This is allowed and usually means that at least one required "
                "paper node (often pressure) is unavailable/non-finite. "
                "Track P will be written with zero samples; Track F will still "
        ...
        pressure_info = pressure_gap_audit(
            raw,
            key,
        )

        # Additional diagnostic: Track-P requires all seven base variables.
        base_valid = np.ones(
            len(
                raw.time_ns
            ),
        ...
        pressure_info[
            "all_7_base_variables_valid_fraction"
        ] = float(
            np.mean(
                base_valid
            )
