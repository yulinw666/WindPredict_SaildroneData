...
def plot_fig5(
    ...
    """
    Final Fig. 5: frozen SD1033 external-generalization comparison.

    Visual convention is intentionally kept identical to Fig. 4:
      Physics-Compact  blue diamond
      DLinear          orange circle
      TimeMixer        green square
      iTransformer     red triangle-up
      PatchTST         purple triangle-down

    All methods use solid lines.  Thin lines and compact markers are used
    because the figure contains three panels.
    """
    ...
    # Same palette/markers as Fig. 4.
    fig5_colors = {
        "Physics-Compact": "#1f77b4",  # blue
        "DLinear": "#ff7f0e",          # orange
        "TimeMixer": "#2ca02c",        # green
        "iTransformer": "#d62728",     # red
    ...
    common_linewidth = 0.90
    common_markersize = 3.65
    common_markeredgewidth = 0.45
    ...
    for j, (dataset_key, title) in enumerate(datasets):
        ...
        # Fallback for files whose model column contains "15E-VH".
        if prop.empty:
        ...
        if prop.empty:
            raise RuntimeError(
                f"Fig. 5: final Physics-Compact results not found "
        ...
        for name in plot_order:
            ...
            if d.empty:
                raise RuntimeError(
                    f"Fig. 5: no horizon data for {name} "
                    f"on {dataset_key}"
                )

            x = d["horizon_min"].to_numpy(float)
            y = d["value"].to_numpy(float)
            yerr = d["std"].to_numpy(float)

            # Proposed model keeps its five-seed error bars.
            if name == "Physics-Compact" and np.any(yerr > 0):
                ax.errorbar(
                    x,
                    y,
                    yerr=yerr,
                    color=fig5_colors[name],
                    marker=fig5_markers[name],
                    linestyle="-",
                    linewidth=common_linewidth,
                    markersize=common_markersize,
                    markeredgewidth=common_markeredgewidth,
                    elinewidth=0.58,
                    capsize=1.4,
                    label=name,
                    zorder=5,
    ...
    # One shared legend above the three panels.
    legend_order = [
        "Physics-Compact",
        "DLinear",
        "TimeMixer",
