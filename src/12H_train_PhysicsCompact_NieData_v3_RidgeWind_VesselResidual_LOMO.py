# -*- coding: utf-8 -*-
r"""
12H_train_PhysicsCompact_NieData_v3_RidgeWind_VesselResidual_LOMO.py

Stage 12H
=========
Development-only leave-one-mission-out (LOMO) training/validation for
Physics-Compact-NieData v3 using the point-sampled Track-J dataset.

DATA FIREWALL
-------------
This script reads only Antarctic, Atlantic and West Coast development missions.
It does not load Tropical_Atlantic_TEST.npz.
...
Thus the wind branch is exactly the fold-specific frozen Ridge forecast. The
GRU residual branch can modify only future vessel velocity.

Architecture:
    GRU hidden = 64, one layer
    dense hidden = 64 + GELU
    dropout = 0.10
    vessel residual head = 2
    vessel gate head = 2

The vessel residual head is initialized to zero, so epoch 0 equals the complete
Ridge forecast.

Two development-only variants are retained:
    CompactNoPhysicsV3:     lambda_AW = 0
    PhysicsCompactNieDataV3: lambda_AW = 2
...
Because W_hat is frozen to Ridge, the wind MSE is constant with respect to
trainable v3 parameters. Model selection therefore concerns the vessel/apparent-
wind correction, while wind accuracy remains exactly that of Ridge.

LOMO folds:
    held out Antarctic: train Atlantic + West Coast
    held out Atlantic:  train Antarctic + West Coast
    held out West Coast: train Antarctic + Atlantic

The same predefined screening seeds and training settings from the previous
Nie-data-compatible experiments are retained.
"""
...
SCRIPT_VERSION = "0.3.0-PhysicsCompact-NieData-v3-RidgeWind-VesselResidual-LOMO"
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "data"
    / "forecasting"
    / "12H_PhysicsCompact_NieData_v3_RidgeWind_VesselResidual_LOMO_v0_1"
)
...
NEURAL_VARIANTS = {
    "CompactNoPhysicsV3": 0.0,
    "PhysicsCompactNieDataV3": FREEZE.lambda_aw_physics,
}
...
def build_model_class(torch, nn, F):
    class PhysicsCompactNieDataV3(nn.Module):
        """Frozen Ridge wind + gated vessel-only residual around Ridge."""
        ...
        def __init__(self):
            ...
            # Epoch 0 equals the complete frozen Ridge forecast.
        ...
        def forward(self, x, ridge_pred_z):
            ...
            # Wind is never modified by the neural branch.
            pred_w = ridge_pred_z[:, 0, 0:2]
            pred_v = ridge_pred_z[:, 0, 2:4] + gv * dv

            pred = torch.cat([pred_w, pred_v], dim=1)[:, None, :]
            zeros_w = torch.zeros_like(pred_w)

            return {
                "pred_z": pred,
                "wind_delta_z": zeros_w,
                "wind_gate": zeros_w,
                "vessel_delta_z": dv,
                "vessel_gate": gv,
                "wind_correction_z": zeros_w,
                "vessel_correction_z": gv * dv,
            }

    return PhysicsCompactNieDataV3
...
def compute_training_loss(
    ...
    # wind_mse is constant with respect to trainable v3 parameters because
    # the wind branch is the frozen Ridge forecast.
    ...
    residual = torch.mean(out["vessel_correction_z"] ** 2)
    total = (
        direct
...
def train_neural_run(
    *,
    ...
    torch.save({
        ...
        "scientific_role": "development_only_LOMO_checkpoint",
...
def main():
    ...
    output_dir = (
        ...
        / "forecasting"
        / (
            "12H_PhysicsCompact_NieData_v3_RidgeWind_VesselResidual_LOMO_v0_1_DEBUG_FAST"
            if args.debug_fast
            else "12H_PhysicsCompact_NieData_v3_RidgeWind_VesselResidual_LOMO_v0_1"
        )
    )
    ...
    log("=" * 124)
    log("12H - PHYSICS-COMPACT-NIEDATA v3 RIDGE-WIND DEVELOPMENT-ONLY LOMO")
    log("=" * 124)
    ...
    freeze_payload = {
        "stage": "12C",
        "model_name": "Physics-Compact-NieData v3.0",
        "script_version": SCRIPT_VERSION,
        "scientific_status": "V3_DEVELOPMENT_ONLY_LOMO",
        "architecture": {
            "equations": [
                "W_hat_z = W_ridge_z",
                "V_hat_z = V_ridge_z + sigmoid(gV)*DeltaV_z",
            ...
            "model_freeze": asdict(FREEZE),
            "ridge_alpha": RIDGE_ALPHA,
            "delta_head_initialization": "vessel delta starts at zero; wind remains frozen Ridge; epoch 0 equals complete Ridge forecast",
        },
        "variants": {
            "CompactNoPhysicsV3": {
                "lambda_AW": 0.0,
            ...
            "PhysicsCompactNieDataV3": {
                "lambda_AW": FREEZE.lambda_aw_physics,
                "role": "v3_candidate_for_Stage12I",
            },
        },
        "LOMO_screening_seeds": SCREENING_SEEDS,
        "Stage12I_final_seeds": FINAL_STAGE12D_SEEDS,
        "checkpoint_metric": FREEZE.checkpoint_metric,
        "held_out_test_policy": (
            "Tropical Atlantic is not read by Stage 12C. "
            "Architecture and loss are fixed by Stage 12H development-only LOMO for Stage 12I re-evaluation."
        ),
    }
    ...
    with (
        ...
        f.write(
            "12C Physics-Compact-NieData Development-Only LOMO\n"
        )
        f.write("=" * 124 + "\n\n")
    ...
    if full_complete and not method_summary.empty:
        ...
        proposed = method_summary.loc[
            method_summary["method"] == "PhysicsCompactNieDataV3"
        ]
        ...
        if not proposed.empty and not ridge_summary.empty:
            p = proposed.iloc[0]
            ...
            log("")
            log(
                f"PhysicsCompactNieDataV3 skill vs Ridge: "
                f"wind={100*wind_skill:+.2f}% | "
                f"AW={100*aw_skill:+.2f}%"
            )

        log("")
        log(
            "[READY] Full development-only LOMO is complete. "
            "Architecture/loss remain frozen for Stage 12D."
        )
    ...
    log(
        "[POLICY] PhysicsCompactNieDataV3 is the proposed Stage-12I re-evaluation model; "
        "CompactNoPhysicsV3 is ablation only."
    )
    log(f"[DONE] 12C outputs: {output_dir}")
